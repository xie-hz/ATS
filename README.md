...
> **是什么 · 解决什么 · 面向谁**
> ATS 是一套面向中小型科技企业 HR 与用人团队的招聘流程管理系统，将分散在 Excel / 邮件 / IM 中的招聘协作收敛为可追踪、可度量、可审计的一体化平台。
>
> **为什么做**
...
项目当前以**实际上线**为目标推进，后续路线图：

- [ ] **生产部署**：Traefik 反向代理 + HTTPS 证书 + 独立子域名（`api.*` / `dashboard.*`），独立数据库与 Redis 实例
- [ ] **视频面试可用性**：部署 coturn TURN 服务器，保障对称 NAT / 严格防火墙环境下的连通率
- [ ] **可观测性**：Sentry 异常监控（已接入）+ Prometheus / Grafana 业务指标 + 结构化日志聚合
- [ ] **性能与容量**：聊天消息分表（MurmurHash 32 表）与中文搜索（pg_trgm GIN 索引）扩容验证、连接池与缓存调优、全链路压测
- [ ] **安全合规**：接口限流与风控、审计日志完善、数据定期备份与恢复演练
- [ ] **体验扩展**：候选人门户 PWA / 移动端适配、面试官移动端提醒推送
- [ ] **多租户 / SaaS 化**（远期）
