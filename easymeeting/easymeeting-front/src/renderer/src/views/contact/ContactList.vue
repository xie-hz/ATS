...
const search = () => {
  if (!keywords.value) {
    contactList.value = sourceContactList.value
    return
  }
  contactList.value = sourceContactList.value.filter((item) => {
    return item.nickName.includes(keywords.value)
  })
}
...
const delContact = (contactId, status) => {
  proxy.Confirm({
    message: `你确定要${status === 2 ? '删除' : '拉黑'}联系人`,
    okfun: async () => {
      let result = await proxy.Request({
