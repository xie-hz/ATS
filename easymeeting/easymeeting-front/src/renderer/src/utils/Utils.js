...
const convertSecondsToHMS = (seconds, showHours = false) => {
    ...
    return (hours == 0 ? hourStr : hours.toString().padStart(2, '0') + ":") + minutes.toString().padStart(2, '0') + ":" + remainingSeconds.toString().padStart(2, '0');
}
...
const format2Html = (content) => {
    ...
    content = content.replace(reg, function (a) {
        ...
        if (!a.indexOf('http')) {
            return '<a class="link-url" href="' + a + '" target=_blank>' + a + '</a>';
        }
        else {
            return '<a class="link-url" href="http://' + a + '" target=_blank>' + a + '</a>';
        }
    });
    return content;
}
...
const getResroucePath = ({ messageId, thumbnail = false, fileType, sendTime }) => {
    return `${import.meta.env.PROD ? import.meta.env.VITE_DOMAIN : ''}${Api.getResource}?messageId=${messageId}&fileType=${fileType}&sendTime=${sendTime}&thumbnail=${thumbnail}&token=${getToken()}`
}
...
const formatMeetingNo = (meetingNo) => {
    return meetingNo.substring(0, 3) + " " + meetingNo.substring(3, 6) + " " + meetingNo.substring(6, 10);
}
